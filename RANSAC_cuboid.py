...
def boundTest(X, Y, Z, pp, corners):
    ...
    A = np.sum(np.multiply(pp, np.tile(d1, (np.size(X), 1))), axis=1)
    B = np.sum(np.multiply(pp, np.tile(d2, (np.size(X), 1))), axis=1)
...
def dist2plane(po, para):
    ...
    dz = abs(np.sum(np.transpose(po).conj() * np.tile(np.vstack(n), (1, rows)), axis=0) - d)
    dz = dz.reshape((-1, 1))
    map = dz <= d
    map = map + (-1) * np.where(map == 0, 1, 0)
    pp = po + np.multiply(np.tile(np.multiply(dz, map), (1, 3)), np.tile(n, (rows, 1)))
...
def para2corner(para):
    ...
    corners = np.tile(centre, (8, 1)) + np.matmul(map, (np.multiply(R, (0.5 * np.tile(np.vstack(scale), (1, 3))))))
