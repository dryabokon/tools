#https://github.com/sunset1995/py360convert/blob/master/py360convert/utils.py
import numpy as np
...
class Panoramer:
    ...
    def equirect_facetype(self, h, w):
        '''
        0F 1R 2B 3L 4U 5D
        '''
    ...
    def xyzpers(self,h_fov, v_fov, u, v, out_hw, in_rot):
        ...
        Rx = self.rotation_matrix(v, [1, 0, 0])
        Ry = self.rotation_matrix(u, [0, 1, 0])
        Ri = self.rotation_matrix(in_rot, np.array([0, 0, 1.0]).dot(Rx).dot(Ry))
    ...
    def sample_cubefaces(self,cube_faces, tp, coor_y, coor_x, order):
    ...
    def cube_h2dict(self,cube_h):
        cube_list = self.cube_h2list(cube_h)
        return dict([(k, cube_list[i])
                     for i, k in enumerate(['F', 'R', 'B', 'L', 'U', 'D'])])

    def cube_dict2h(self,cube_dict, face_k=['F', 'R', 'B', 'L', 'U', 'D']):
        assert len(face_k) == 6
        return self.cube_list2h([cube_dict[k] for k in face_k])

    def cube_h2dice(self,cube_h):
        assert cube_h.shape[0] * 6 == cube_h.shape[1]
        w = cube_h.shape[0]
    ...
    def cube_dice2h(self,cube_dice):
    ...
    def rotation_matrix(self,rad, ax):
        ax = np.array(ax)
    ...
    def cubemap2equirect(self,cubemap, h, w, mode='bilinear', cube_format='dice'):
        if mode == 'bilinear':
            order = 1
        elif mode == 'nearest':
            order = 0
        ...
        if cube_format == 'horizon':
            ...
            cubemap = self.cube_list2h(cubemap)
        elif cube_format == 'dict':
            cubemap = self.cube_dict2h(cubemap)
        elif cube_format == 'dice':
            cubemap = self.cube_dice2h(cubemap)
        else:
        ...
        equirec = np.stack([
            self.sample_cubefaces(cube_faces[..., i], tp, coor_y, coor_x, order=order)
            for i in range(cube_faces.shape[3])
        ], axis=-1)

        return equirec

    def equirect2cubemap(self, e_img, side=256, mode='bilinear', cube_format='dice'):
        '''
        ...
        xyz = self.xyzcube(side)
        uv = self.xyz2uv(xyz)
        coor_xy = self.uv2coor(uv, h, w)

        cubemap = np.stack([
            self.sample_equirec(e_img[..., i], coor_xy, order=order)
        ...
        if cube_format == 'horizon':
            ...
            cubemap = self.cube_h2list(cubemap)
        elif cube_format == 'dict':
            cubemap = self.cube_h2dict(cubemap)
        elif cube_format == 'dice':
            cubemap = self.cube_h2dice(cubemap)
        else:
            raise NotImplementedError()

        return cubemap

    def equirect2planar(self,e_img, fov_deg, u_deg, v_deg, out_hw, in_rot_deg=0, mode='bilinear'):
        '''
        e_img:   ndarray in shape of [H, W, *]
        fov_deg: scalar field of view in degree
        ...
        u = -u_deg * np.pi / 180
        v = v_deg * np.pi / 180
        xyz = self.xyzpers(h_fov, v_fov, u, v, out_hw, in_rot)
        uv = self.xyz2uv(xyz)
        coor_xy = self.uv2coor(uv, h, w)
