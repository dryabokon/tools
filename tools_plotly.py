...
# ---------------------------------------------------------------------------------------------------------------------
    def create_figure_test_sales(self):
        ...
        A = numpy.array([['East', 'Houston', 'AX', 42, 9],['North', 'Hartley', 'BI', 24, 8],['West', 'Brewster', 'BS', 33, 14],['West', 'Presidio', 'DQ', 40, 15],['South', 'Duval', 'DS', 42, 9],['North', 'Motley', 'EH', 44, 34],['North', 'Gray', 'ES', 49, 31],['South', 'Duval', 'FJ', 20, 7],['South', 'Zavala', 'FL', 31, 24],['East', 'Tyler', 'GT', 30, 4],['South', 'Duval', 'GW', 20, 6],['East', 'Rusk', 'HW', 20, 8],['North', 'Dallam', 'IJ', 20, 6],['West', 'Brewster', 'IK', 33, 20],['South', 'Zavala', 'IV', 28, 20],['East', 'Tyler', 'JC', 21, 17],['North', 'Dallam', 'JE', 35, 23],['West', 'Brewster', 'JF', 20, 8],['South', 'Webb', 'KR', 20, 6],['South', 'Hidalgo', 'LQ', 41, 16],['South', 'Hidalgo', 'LR', 39, 7],['South', 'Hidalgo', 'LV', 21, 11],['North', 'Motley', 'MW', 22, 21],['North', 'Motley', 'NY', 35, 15],['East', 'Shelby', 'NZ', 45, 16],['North', 'Floyd', 'OH', 31, 6],['West', 'Presidio', 'OT', 35, 9],['East', 'Rusk', 'OY', 40, 20],['North', 'Hartley', 'PL', 42, 37],['South', 'Webb', 'QJ', 46, 40],['East', 'Houston', 'QK', 22, 8],['West', 'Pecos', 'QM', 48, 45],['South', 'Webb', 'RR', 48, 26],['West', 'Presidio', 'SV', 45, 30],['North', 'Moore', 'TW', 33, 6],['North', 'Moore', 'TZ', 33, 20],['North', 'Floyd', 'UA', 44, 34],['East', 'Tyler', 'UF', 49, 35],['East', 'Rusk', 'UM', 24, 8],['East', 'Shelby', 'VN', 29, 6],['North', 'Hartley', 'WE', 39, 37],['North', 'Floyd', 'WH', 44, 37],['West', 'Pecos', 'WV', 30, 25],['West', 'Pecos', 'WZ', 37, 23],['East', 'Shelby', 'XH', 47, 6],['North', 'Gray', 'XY', 31, 28],['South', 'Zavala', 'YA', 25, 21],['North', 'Gray', 'YJ', 35, 13],['East', 'Houston', 'YN', 38, 23],['North', 'Moore,' ,   'ZM', 21, 13],['North', 'Dallam', 'ZQ', 49, 13]])
        ...
        df_hierarchical = tools_DF.build_hierarchical_dataframe(df, cols_labels_level=['salesperson', 'county', 'region'], col_size='calls',dct_function=dct_function )

        dct_marker_colors =dict(colors=df_hierarchical['metric'], colorscale='RdBu')
        fig = go.Sunburst(labels=df_hierarchical['id'],parents=df_hierarchical['parent_id'],values=df_hierarchical['size'],
            branchvalues='total',
        ...
        return fig
# ---------------------------------------------------------------------------------------------------------------------
    def create_figure_sunbust(self, df_hierarchical, colorscale='viridis', metric_name='', height=600, do_treemap=False):
        ...
        dct_marker_colors = dict(colors=df_hierarchical['metric'], colorscale=colorscale, line=dict(color='#222222',width=1))
        hovertemplate = None
        if metric_name!='':
        ...
        if do_treemap:
            fig = go.Treemap(labels=df_hierarchical['id'],parents=df_hierarchical['parent_id'],values=df_hierarchical['size'],branchvalues='total',marker=dct_marker_colors,hovertemplate=hovertemplate,hoverinfo='none')
        else:
            fig = go.Sunburst(labels=df_hierarchical['id'],parents=df_hierarchical['parent_id'],values=df_hierarchical['size'],branchvalues='total',marker=dct_marker_colors,hovertemplate=hovertemplate,hoverinfo='none')
...
# ----------------------------------------------------------------------------------------------------------------------
    def create_figure_squarify(self, labels, sizes, colors=None, W=200, H=200):
        ...
        hoverinfo = 'none'
        fig = go.Figure(go.Scatter(x=rect_x, y=rect_y,fill="toself",hoverinfo=hoverinfo,mode = 'lines',marker=dct_marker_colors,opacity=1.0,showlegend=False))
