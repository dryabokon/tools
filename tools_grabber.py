...
class Grabber:
    ...
    def print_debug_info(self,image, font_size=18):
        ...
        space = font_size + 20
        N = len(self.frame_buffer) if len(self.frame_buffer) >0 else self.max_frame_id
        label = '%06d / %06d | %.1f sec | %.1f fps'%(self.last_frame_given,N,self.HB.get_delta_time(), self.HB.get_fps())
    ...
    def capture_filenames(self):
        ...
        self.cap = None
        self.df_filenames = pd.DataFrame({'filename':tools_IO.get_filenames(self.source, '*.jpg,*.png') })
        self.df_filenames['frame_id'] = numpy.arange(0, 0 + self.df_filenames.shape[0])
        self.is_initiated = True
    ...
    # ---------------------------------------------------------------------------------------------------------------------
    def get_frame_filenames(self,frame_id=None):

        if frame_id is None:
            if self.last_frame_given + 1 < self.max_frame_id:
                self.last_frame_given += 1
                frame_id = self.last_frame_given
            else:
                if self.looped:
                    self.last_frame_given = 0
                    frame_id = 0
                else:
                    frame_id = self.max_frame_id
        else:
            #frame_id = min(self.max_frame_id-1,frame_id)
            frame_id = frame_id % int(self.max_frame_id)

        filename = self.df_filenames[self.df_filenames['frame_id'] == frame_id]['filename'].values[0]
        image = cv2.imread(self.source + filename)
        return image
    # ---------------------------------------------------------------------------------------------------------------------
    ...
    def get_frame(self,frame_id=None):
        ...
        elif self.mode== 'filenames':return self.get_frame_filenames(frame_id)
