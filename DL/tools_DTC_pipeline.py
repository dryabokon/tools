...
# ----------------------------------------------------------------------------------------------------------------------
class Pipeliner:
    def __init__(self,folder_out,config,Grabber=None):
        ...
        #colormap = 'cool'
        #self.colors80 = tools_draw_numpy.get_colors(80, colormap=colormap, shuffle=True)
        self.colors80 = [tools_draw_numpy.color_amber]*80
        self.frame_buffer = []
        self.ROI = None
    ...
    def draw_detects(self, image, rects, labels=None, colors=(0, 128, 255),w=1):
        ...
        for rect, label,color in zip(rects, labels,colors):
            col_left, row_up, col_right, row_down = rect
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    def process_frame_as_mask(self,filename,do_debug=True):
        frame = cv2.imread(filename) if isinstance(filename, str) else filename
        y_pred = self.Detector.predict_one(frame / 255.0)
        image_debug = cv2.resize(y_pred, (frame.shape[1], frame.shape[0]))
        return image_debug
    # ----------------------------------------------------------------------------------------------------------------------
    def process_frame(self, filename,do_debug=False):
        ...
        self.TP.tic('track')
        self.df_det_frame = df_track_frame_filtered

        self.TP.tic('track Siam')
        if self.config.do_tracking_siam:
            if (self.ROI is None) and (not self.Tracker_SiamFC.is_initialized) and (self.df_det_frame is not None) and (self.df_det_frame.shape[0] > 0):
                self.ROI = self.df_det_frame[['x1','y1','x2','y2']].values[0]
            df_pred_siam = self.Tracker_SiamFC.track_detections(self.ROI, frame, frame_id)
            if self.Tracker_SiamFC.is_initialized and df_pred_siam is not None and df_pred_siam.shape[0] > 0:
                self.ROI = df_pred_siam[['x1','y1','x2','y2']].values[0]
        self.TP.tic('track Siam')
    ...
    def process_video(self,save_images=None):
        ...
        for i in tqdm(range(int(N)), desc='processing video'):
            ...
            if self.config.do_tracking:
                image = self.process_frame(image,do_debug)
