...
# ----------------------------------------------------------------------------------------------------------------------
    def track_detections(self,df_det,filename_in,frame_id=None,do_debug=False):
        ...
        if res.boxes.cpu().numpy().shape[0]>0:
            rects = res.boxes.xyxy.cpu().numpy()
            class_ids = res.boxes.cls.cpu().numpy()
            confs = res.boxes.conf.cpu().numpy()
            if res.boxes.id is not None:
                track_ids = res.boxes.id.cpu().numpy()
            else:
                track_ids = numpy.full((rects.shape[0]),-1, dtype=int)

        df_track = pd.DataFrame(numpy.concatenate((track_ids.reshape((-1, 1)), class_ids.reshape((-1, 1)),rects.reshape((-1, 4)), confs.reshape(-1, 1)), axis=1),columns=['track_id', 'class_ids','x1', 'y1', 'x2', 'y2', 'conf'])
